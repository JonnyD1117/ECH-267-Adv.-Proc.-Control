...
for k=1:N
    ...
    obj = obj + (st - ref)'*Q*(st - ref) + (con- con_ref)'*R*(con-con_ref) + 5000*(norm(max(c,0))); % Construct Symbolic representation of objective function at time step "K"   
...
% PID function 
function y  = MIMO_PID_SYS(sys1_u1, sys1_u2, sys2_u1, sys2_u2, u1, u2, x0, dt)

    x1_u1_2 = sys1_u1*(u1) + sys1_u2*(u2);
    x2_u1_2 = sys2_u1*(u1) + sys2_u2*(u2);

    y = lsim([x1_u1_2;x2_u1_2], [1,1], dt, x0);
