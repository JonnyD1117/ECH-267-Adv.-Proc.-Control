...
phi_x_plus = h*sin(deg2rad(15)); 
phi_x_minus= -phi_x_plus; 

phi_y_plus = -h*cos(deg2rad(15)); 
phi_y_minus= phi_y_plus; 

plot([0,phi_x_plus], [0,phi_y_plus],'k')
plot([0,phi_x_minus], [0,phi_y_minus],'k')


fill([0 phi_x_plus phi_x_minus], [0 phi_y_plus phi_y_minus ], [.75, .73,.70]); % Clears Background


l1= .044;

beta = .5*pi - atan(((-y1)/(-x1)));

d_x2_new_p = l1*cos(beta);
d_x2_new_m = -l1*cos(beta);
d_y2_new_p = l1*sin(beta);
d_y2_new_m = -l1*sin(beta);


P1 = [0+d_x2_new_m,0+d_y2_new_p];
P2 = [0+d_x2_new_p,0+d_y2_new_m]; 
...
plot([x1,P1(1)], [y1,P1(2)],'k')
plot([x1,P2(1)], [y1,P2(2)],'k')


fill([x1 P1(1) P2(1)], [y1 P1(2) P2(2)], [.75, .73,.70]); % Clears Background
...
function q_ddot = RobotModel_q_ddot(p, q1, q2, q1_dot, q2_dot,tau1, tau2)
