...
q0 = [0;0;.1];
q0_dot = [0;0;0];
q0_ddot = NEW_puma_560(q0, q0_dot, tau);


for i=2:1:100
    
        q0_ddot(:,i) = NEW_puma_560(q0(:,i-1), q0_dot(:,i-1), tau);
...
function q_ddot = NEW_puma_560(theta, theta_dot, tau)
...
v1 = 0.3*(q2_dot + q3_dot)*sin(q2 + q3)*q3_dot - 1.8*sin(q2)*cos(q2)*q1_dot*q2_dot  + 1.92*cos(q2)*q2_dot^2;
v2 = - 0.3*sin(q2 + q3)*q1_dot*q3_dot + 0.9*sin(q2)*cos(q2)*q1_dot^2;
v3 = 0.3*(q2_dot + q3_dot)*sin(q2 + q3)*q1_dot - 0.3*sin(q2 + q3)*q1_dot*q3_dot;


g1 = 0;
g2 = - 3.3*sin(q2)*cos(q3) + 31.9*sin(q2) - 3.3*sin(q3)*cos(q2);
g3 = - 3.3*sin(q2)*cos(q3) - 3.3*sin(q3)*cos(q2);
