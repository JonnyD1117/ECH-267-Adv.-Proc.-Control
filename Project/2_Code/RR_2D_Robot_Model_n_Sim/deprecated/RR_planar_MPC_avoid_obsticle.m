...
for k = 1:N
    ...
    pos_cg = CG_forward_kinematics(p,[X(1,k);X(2,k)], [X(3,k);X(4,k)]);
    
    x = pos_cg.x;
    y = pos_cg.y; 
    
    c1 = -sqrt((x(1) - obj_x)^2 + ((y(1) - obj_y)^2)) + (.125 + obj_dia);
    c2 = -sqrt((x(2) - obj_x)^2 + ((y(2) - obj_y)^2)) + (.125 + obj_dia);
...
% Object Constraints
args.lbg(4*(N+1) + 5:2:4*(N+1) + 4 + 2*N) = -inf; % Link 1 Constraints 
args.ubg(4*(N+1) + 5:2:4*(N+1) + 4 + 2*N) = 0; 

args.lbg(4*(N+1) + 6:2:4*(N+1) + 4 + 2*N) = -inf; % Link 1 Constraints 
args.ubg(4*(N+1) + 6:2:4*(N+1) + 4 + 2*N) = 0; 
...
for i=1:1:length(x_list)
    ...
    x_vec = pos.x;
    y_vec = pos.y;
    plot_robot_with_object(x_vec, y_vec, obj_x, obj_y)
...
for k=1:N
   ...
   c = -sqrt((x - obj_x )^2 + (y - obj_y)^2) + (end_effect_dia + obj_dia);
    ...
    obj = obj + (st - ref)'*Q*(st - ref) + (con- con_ref)'*R*(con-con_ref) + 50000*(max(c,0)); % Construct Symbolic representation of objective function at time step "K"   
