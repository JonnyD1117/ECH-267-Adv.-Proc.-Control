...
plot(x_list(1,:));
ylabel('Theta [rads]'); 
title('MPC: Stabilizing Controller X_0 = [0,0,0,0]')
...
ylabel('Torque (N*m)')
title('MPC: Actuation Effort (Stabilizing Controller)')
...
for i=1:1:length(x_list)
    ...
    theta(:,i) = x_list(1:2,i); 
    theta_dot = x_list(3:4,i); 
%     pos = forward_kinematics(p,theta(:,i), theta_dot);
%     
%     x_vec = pos.x;
%     y_vec = pos.y;
%       
...
t_traj = linspace(2,4, 100);

q1_spline = spline(theta(1,:), t, t_traj);

% plot(t, theta(1,:),'o', t_traj, q1_spline)
% plot(t_traj, q1_spline)
